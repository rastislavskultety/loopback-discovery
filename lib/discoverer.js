...
Discoverer.prototype.normalizeSchema = function(schema) {
  var idProps = [];
  var idNames = [];
  ...
  for (var pname in schema.properties) {
    ...
    customizeProperty(prop);
  }

  // by default the tables should have only one primary key (identity) with name 'id'
  var defaultId = (idNames.length === 1 && idNames[0] === 'id');

  if (defaultId) {
    // make id generated and not required
    idProps[0].generated = true;
    delete idProps[0].required;
  }

  // schema.idInjection = true;
  schema.forceId = defaultId;
  schema.strict = true;
}


Discoverer.prototype.createJSONSchema = function(model) {
  var schema = JSON.parse(JSON.stringify(model));
  // move settings property to root
  if (schema.settings) {
    Object.assign(schema, schema.settings);
...
Discoverer.prototype.saveModel = function(modelsPath, model, overwrite, cb) {
  try {
    ...
    if (!utils.fileExists(modelPath) || overwrite === true) {
      ...
      fs.writeFileSync(modelPath, this.createJSONSchema(model));
      cb(null, model);
    } else {
  ...
  self.dataSource.connector.discoverModelDefinitions(options, function(err, tables) {
    if (err) {
      ...
      async.mapSeries(tables, function(table, callback) {
        self.dataSource.discoverAndBuildModels(table.name, {
          relations: false
        }, function(err, models) {
          if (err !== null) {
            callback(err);
...
// make custom transformations of properties
function customizeProperty(prop) {
  // convert MySQL TEXT type to "object" type of model
  if (prop.mysql && prop.mysql.dataType === 'text') {
    prop.type = 'object';
    delete prop.length;
  ...
  if (prop.mysql && prop.mysql.dataType === 'tinyint') {
    prop.type = 'boolean';
  }
}
